 ...
 class OrdenTrabajo extends Conectar{
        ...
        public function update_ordent($ordt_id, 
         ...
         public function insert_comisiones_orden(
            ...
            $ordt_id, $suc_id, $tec_id, $tec_nom, $comision_porc, $sub_total, $comision_valor, ){
            $conectar=parent::Conexion();
            $sql="call SP_INSERT_COMISIONES_02(?,?,?,?,?,?,?)";
            $query=$conectar->prepare($sql);
            $query->bindValue(1,$ordt_id);
            $query->bindValue(2,$suc_id);
            $query->bindValue(3,$tec_id);
            $query->bindValue(4,$tec_nom);
            $query->bindValue(5,$comision_porc);
            $query->bindValue(6,$sub_total);
            $query->bindValue(7,$comision_valor);
            $query->execute();
